...
-- why doesnt rfl resolve??
example : @List.merge_nodups _ inferInstance ([] : List ℕ) ([]) = [] := by sorry
...
def Hyperlist.merge_nodups_for_zipWith_zeros
  ...
  match is1, is2 with
  | [], [] => rw[List.merge_nil]; exact f as bs
  | [], i2 :: is2 =>
    match bs with
    | [] => rw[List.merge_nil]; exact []
    | b :: bs => rw[List.merge_nil]; exact (by rw[←@List.merge_nil _ _ is2]; exact merge_nodups_for_zipWith_zeros [] is2 f as b) :: Hyperlist.map (f 0) (hlc bs)
  | i1 :: is1, [] =>
    match as with
    | [] => rw[List.merge_nil']; exact []
    | a :: as => rw[List.merge_nil']; exact (by rw[←@List.merge_nil' _ _ is1]; exact merge_nodups_for_zipWith_zeros is1 [] f a bs) :: Hyperlist.map (fun x => f x 0) (hlc as)
  | i1 :: is1, i2 :: is2 =>
...
theorem List.heq_of {α β : Type} {l : List α} {l' : List β} (h : l.length = l'.length)
    (h' : HEq α β) (h'' : ∀ i : Fin l.length, HEq (l[i]) (l'[i])) : HEq l l' := by
  simp [eq_of_heq] at h'
  subst h'
  simp; apply ext_get; assumption; intro i; intro h1 h2; apply eq_of_heq; apply h'' ⟨i, h1⟩

example :
    Hyperlist.merge_nodups_for_zipWith_zeros [0] [0] (. + .) [hl0 6, hl0 9] [hl0 7] =
    (by simp; exact[hl0 13, hl0 9]):= by
  simp [compare, compareOfLessAndEq, cast_eq_iff_heq, List.heq_of]; apply List.heq_of; simp; intro i; fin_cases i <;> simp; rfl

@[simp]
noncomputable def Hyperlist.expand
  [Semiring γ] (C : R → γ) (X : σ → γ) :
